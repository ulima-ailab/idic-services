...
def export_interruptibility_raw(request):
    ...
    for index, row in df_app_data.iterrows():
        ...
        while True:
            if row["id_user"] == df_web_data.iloc[curr_idx]["id_user"]:
                ...
                start_ts_ = row["delivery_time"] # datetime.fromtimestamp(row["delivery_time"])
                end_ts = df_web_data.iloc[curr_idx]["timestamp"] # datetime.fromtimestamp(df_web_data.iloc[curr_idx]["timestamp"])
    ...
    return JsonResponse({"msg": "data exported"}, safe=False)
